...
# Код берётся из образа (нет монтирования). После make frontend-build контейнер НЕ видит новые JS/CSS.
# Когда: деплой. Для разработки и правок фронта используйте make up-dev.
up:
	$(COMPOSE) up -d

# up-dev — то же, но с монтированием кода с хоста (режим разработки).
# make frontend-build на хосте сразу отражается в контейнере (public/assets общий).
# Когда: локальная разработка, правки JS/CSS, проверка сборки фронта.
up-dev:
	$(COMPOSE_DEV) up -d

# stop — остановить контейнеры.
# Когда: нужно временно остановить контейнеры без удаления.
stop:
	$(COMPOSE) stop
...
# Перед сборкой очищается кэш webpack, чтобы правки в JS гарантированно попали в бандл.
