<h1 class="mt-0 mb-4 lh-1">Изменить тип оборудования</h1>
<form class="form" action="<?= base_url('/admin/room-equipment/edit/' . $roomEquipment['id']) ?>" method="post">
    <?= getCsrfField(); ?>
    <div class="form__group d-flex flex-column">
        ...
        <select class="form__select <?= getValidationClass('room_id') ?>" name="room_id" id="room_id">
            <? foreach ($rooms as $room): ?>
                <option value="<?= $room['id'] ?>" <?= $room['id'] == $roomEquipment['room_id'] ? 'selected' : '' ?>>
                    <?= $room['name'] ?>
                </option>
            <? endforeach ?>
        </select>
    ...
    <div class="form__group d-flex flex-column">
        ...
        <select class="form__select <?= getValidationClass('equipment_id') ?>" name="equipment_type_id" id="equipment_type_id">
            <? foreach ($equipmentTypes as $equipment): ?>
                <option value="<?= $equipment['id'] ?>" <?= $equipment['id'] == $roomEquipment['equipment_type_id'] ? 'selected' : '' ?>>
                    <?= $equipment['name'] ?>
                </option>
            <? endforeach ?>
        </select>
    ...
    <div class="form__group d-flex flex-column">
        ...
        <input class="form__input" type="number" name="quantity" id="quantity" value="<?= $roomEquipment['quantity'] ?>">
        <?= getErrors('quantity') ?>
    </div>
    <div class="form__group d-flex flex-column">
        <label class="form__label" for="notes">Описание</label>
        <input class="form__input" type="text" name="description" id="notes" value="<?= $roomEquipment['notes'] ?>">
        <?= getErrors('notes') ?>
    </div>
